...
function getBalanceIndex(arr) {
  ...
  for (let balanceIndex = 1; balanceIndex < arr.length - 1; balanceIndex += 1) {
    ...
    for (let i = 0; i < arr.length; i += 1) {
      if (i < balanceIndex) sumLeft += arr[i];
      if (i > balanceIndex) sumRight += arr[i];
    }
    if (sumLeft === sumRight) return balanceIndex;
  }
  return -1;
}
...
function getSpiralMatrix(size) {
  ...
  while (startCol <= endCol && startRow <= endRow) {
    for (let i = startCol; i <= endCol; i += 1) {
      ourMatrix[startRow][i] = count;
      count += 1;
    }
    startRow += 1;

    for (let i = startRow; i <= endRow; i += 1) {
      ourMatrix[i][endCol] = count;
      count += 1;
    }
    endCol -= 1;

    for (let i = endCol; i >= startCol; i -= 1) {
      ourMatrix[endRow][i] = count;
      count += 1;
    }
    endRow -= 1;

    for (let i = endRow; i >= startRow; i -= 1) {
      ourMatrix[i][startCol] = count;
      count += 1;
    }
    startCol += 1;
  }
...
function rotateMatrix(matrix) {
  const matrixI = matrix.length - 1;
  const matrixJ = matrix[0].length - 1;
  const answer = [];

  for (let i = 0; i <= matrixI; i += 1) {
    const innerArr = [];
    for (let j = 0; j <= matrixJ; j += 1) {
      innerArr[j] = 1;
    }
    answer[i] = innerArr;
  }
  ...
  while (count <= matrixI) {
    for (let j = 0; j <= matrixJ; j += 1) {
      answer[currentRow][colNumber] = matrix[rowNumber][j];
      currentRow += 1;
    }
    rowNumber += 1;
    currentRow = 0;
...
function sortByAsc(arr) {
  ...
  const result = [...leftArr, base, ...rightArr];
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 1; i <= iterations; i += 1) {
    ...
    ourString = evenStr + oddStr;
    const residueIterations = iterations % i;
    if (ourString === str) return shuffleChar(str, residueIterations);
  }
